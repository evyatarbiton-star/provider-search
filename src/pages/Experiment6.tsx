    ...
    <div ref={ref} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      {open && (
        <div style={{
          ...
          padding: SPACE_S, minWidth: 200, zIndex: 9000,
        }}>
          {children}
        </div>
      )}
...
export function Experiment6() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG_WHITE }}>
      ...
      {/* Filter bar removed for Experiment 6 — map-focused layout */}

      {/* ── Main Content ── */}
      <div style={{
        maxWidth: 1280, margin: '0 auto',
        display: 'flex', flexDirection: 'row', gap: 56,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {/* Title + Sort removed for map-focused layout */}
          ...
          <div style={{
            ...
            overflow: 'hidden',
            border: `1px solid ${BORDER_STRONG}`,
            height: showSidebar ? 'calc(100vh - 220px)' : 400,
            position: showSidebar ? 'sticky' : 'relative',
            top: showSidebar ? 200 : undefined,
          }}>
            {!isLoading && (
              <MapContainer
                center={[40.7505, -73.9780]}
                zoom={13}
              ...
              >
                ...
                {providers.map((provider, i) => {
                  const coords = PROVIDER_COORDS[provider.address] || [40.75 + (Math.random() - 0.5) * 0.03, -73.98 + (Math.random() - 0.5) * 0.03]
                  const price = provider.cost || '?'
                  return (
                    <Marker
                      key={i}
                      position={coords}
                      icon={createPriceIcon(price, provider.costLevel)}
                    >
                      <Popup>
                        <div style={{ fontFamily: FONT, minWidth: 200 }}>
                          <p style={{ fontWeight: W_MEDIUM, fontSize: fontSizes[16], margin: '0 0 4px', color: TEXT_DEFAULT }}>{provider.name}</p>
                          <p style={{ fontSize: fontSizes[14], margin: '0 0 8px', color: TEXT_DARK }}>{provider.specialty}</p>
                          <p style={{ fontSize: fontSizes[14], margin: '0 0 4px', color: TEXT_DARK }}>⭐ {provider.rating}/5 ({provider.reviewCount})</p>
                          <p style={{ fontSize: fontSizes[14], margin: '0 0 8px', color: TEXT_DARK }}>{provider.address}</p>
                          {provider.cost && (
                            <p style={{ fontWeight: W_MEDIUM, fontSize: fontSizes[18], margin: '0 0 4px', color: TEXT_DEFAULT }}>{provider.cost} <span style={{ fontSize: fontSizes[14], fontWeight: W_REGULAR, color: TEXT_DARK }}>{provider.costLabel}</span></p>
                          )}
                          {provider.nextAppointmentDate && (
                            <p style={{ fontSize: fontSizes[14], margin: '8px 0 0', color: TEXT_DARK }}>📅 Next: <strong>{provider.nextAppointmentDate}</strong></p>
                          )}
                        </div>
                      </Popup>
                    </Marker>
            ...
            {isLoading && (
              <div style={{
                width: '100%', height: '100%', background: BG_SUBTLE,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: FONT, color: TEXT_LIGHT,
              }}>
                Loading map...
              </div>
            )}
